...
-- Момент, до которого пользователю уже проиграли анимацию «Обмен принят».
-- Обмены с resolved_at позже — «непоказанные»: клиент проиграет их при заходе
-- или на опросе. DEFAULT now(): прошлые обмены существующим/новым не переигрываем.
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_swap_seen_at TIMESTAMPTZ DEFAULT now();
...
async def accept_trade_offer(offer_id: int, market_fee: float = 0.0) -> str:
    ...
    async with pool.acquire() as con:
        async with con.transaction():
            ...
            # Принимающий (владелец лота) видит анимацию «Обмен принят» сразу в
            # UI — помечаем ему обмен показанным (resolved_at == этот NOW()),
            # чтобы поллер не проиграл её второй раз. Предложившего НЕ трогаем:
            # он получит анимацию при следующем заходе/опросе.
            await con.execute(
                "UPDATE users SET last_swap_seen_at = NOW() WHERE user_id=$1",
                offer["to_user_id"],
            )
    return ""
...
async def get_and_mark_unseen_swaps(user_id: int) -> list:
    """Завершённые обмены пользователя, которые ему ещё НЕ показывали
    (resolved_at позже его last_swap_seen_at). Атомарно помечает их
    показанными — двигает last_swap_seen_at до самого свежего из возвращённых
    (фильтр строго `>`, поэтому тот же обмен повторно не вернётся, а новый —
    вернётся). Данные те же, что у get_user_completed_trades, плюс id обеих
    сторон, чтобы клиент сориентировал «мои подарки слева»."""
    pool = await get_pool()
    async with pool.acquire() as con:
        async with con.transaction():
            seen = await con.fetchval(
                "SELECT last_swap_seen_at FROM users WHERE user_id=$1 FOR UPDATE", user_id)
            rows = await con.fetch(
                f"""SELECT o.offer_id, o.resolved_at as completed_at, o.top_up_ton, o.fee_ton,
                   ...
                   JOIN users uto ON t.owner_id = uto.user_id
                   WHERE o.status='accepted' AND (o.from_user_id=$1 OR t.owner_id=$1)
                     AND ($2::timestamptz IS NULL OR o.resolved_at > $2)
                   ORDER BY o.resolved_at ASC""",
                user_id, seen,
            )
            if rows:
                newest = max(r["completed_at"] for r in rows)
                await con.execute(
                    "UPDATE users SET last_swap_seen_at=$1 WHERE user_id=$2", newest, user_id)
    result = []
    for r in rows:
        d = dict(r)
        d["target_gifts"] = _parse_json_list(d["target_gifts"])
